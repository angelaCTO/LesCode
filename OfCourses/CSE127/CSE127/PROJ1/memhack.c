...
/*-- UNNECESSARY ------------------------------------------------------------*/
	...
	// Loop though the length of the guess buffer and guess each character
	for (i = 0; i < BUFFLEN; i++) {
		// Naive iteration through all possible ASCII characters
		...
		// Do we have the right guess ? Lets hope so...
		if (check_pass(guess)) {
