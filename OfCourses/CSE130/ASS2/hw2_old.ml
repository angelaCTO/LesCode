...
let addNew x l1 l2 = 
        if (not (List.mem x l1) && List.mem x l2) then [x] @ l1
        else l1 ;;
...
let removeDuplicates l = 
  let rec helper (seen,rest) = 
      match rest with 
      | []   -> seen
      | h::t -> let seen' = addNew h seen rest in
                let rest' = t in 
	        helper (seen',rest')
 ...
 *)
let fixpoint (f,b) = wwhile((fun b -> let b' = (f b) in (b', b' != b)), b) ;;
...
let rec exprToString e = 
        match e with
          VarX 	     	           -> "x"
	| VarY 	    	           -> "y"
  	| Sine	   e1	           -> Printf.sprintf "sin(pi * %s)" 
				      (exprToString e1)
  	| Cosine   e1  	           -> Printf.sprintf "cos(pi * %s)"
				      (exprToString e1)
  	| Average (e1, e2)         -> Printf.sprintf "((%s + %s) / 2)"
				      (exprToString e1) (exprToString e2)
  	| Times   (e1, e2)         -> Printf.sprintf "(%s * %s)"
				      (exprToString e1) (exprToString e2) 
  	| Thresh  (e1, e2, e3, e4) -> Printf.sprintf "(if %s < %s then %s else %s)"
				      (exprToString e1) (exprToString e2) 
...
(* TODO eval : expr -> float * float -> float 
...
let rec eval (e,x,y) =
	...
	| VarY 	    	           -> y
  	...
  	| Times   (e1, e2)         -> ((eval(e1, x, y)) *. (eval(e1, x, y)))
...
(* TODO build: (int*int->int) * int -> Expr 
...
let rec build (rand, depth) = 
        if depth < 0 
        then let x1 = rand(0,1) in 
        match x1 with 
         0 -> buildX ()
        |1 -> buildY ()
        else let x2 = rand(0,6) in
	match x2 with
         0 -> buildX ()
	|1 -> buildY ()
...
(* TODO g1,g2,g3,c1,c2,c3 : unit -> int * int * int
