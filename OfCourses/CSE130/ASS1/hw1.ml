...
let rec sumInt n =  
  if n / 10  <= 0 then n
  else (n mod 10) + sumInt (n / 10);;

let rec additivePersistence n = 
  if (sumInt n) < 9 then 1
  else 1 + additivePersistence (sumInt n) ;;
...
let rec listReverse l = 
  match l with 
    | [] -> []
    | elem::subl -> (listReverse subl) @ [elem] ;;
...
let rec compareWords w1 w2 =
  match (w1, w2) with
    | ([], []) -> true
    | ([], h2::t2) -> false
    | (h1::t1, []) -> false
    | (h1::t1, h2::t2) -> h1 = h2 && compareWords t1 t2 ;;

let palindrome w =
  let word_l = explode w in
  let word_rl = listReverse word_l in
    compareWords word_l word_rl ;;
